...
class SidePanel(QWidget):
    ...
    def _init_file_nav_section(self) -> None:
        ...
        self.jump_button.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        self.jump_button.clicked.connect(self._emit_jump)

        jump_layout.addWidget(jump_label)
        jump_layout.addWidget(self.jump_spin)
        jump_layout.addWidget(self.jump_button, 1)  # stretch factor -> fills remaining space
    ...
    def _make_slider(
        ...
        val_label.setStyleSheet("color: #8E8E93;")  # Qt QSS does not support font-variant-numeric

        self._slider_labels[name] = val_label

        slider.valueChanged.connect(
            lambda val, lbl=val_label, nm=name, sc=scale: self._on_adjustment_change(lbl, nm, val, sc)
        )
